...
export async function createSubscriptionDelegation(
  ...
  const env = getSmartAccountsEnvironment(ACTIVATION_CHAIN_ID)
  ...
  const caveats = builder
    ...
    .addCaveat('timestamp', { afterThreshold: validAfter, beforeThreshold: validBefore })
  ...
  const delegation = createDelegation({
    ...
    to: options.delegate,
    parentDelegation: options.parentDelegationHash,
    caveats,
  })
    ...
    caveats: [
      {
        ...
        decodedTerms: { afterThreshold: validAfter, beforeThreshold: validBefore },
      ...
      },
      {
        ...
        decodedTerms: { maxAmount: maxAmount.toString(), token: CONTRACTS.usdc },
